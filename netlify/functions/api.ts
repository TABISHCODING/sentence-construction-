...
/**
 * Netlify Function handler for the API
 */
const handler: Handler = async (event: HandlerEvent, context: HandlerContext) => {
  ...
  if (event.httpMethod === 'GET' && event.path === '/.netlify/functions/api/data') {
    try {
      console.log('Serving questions data');
  ...
  // Handle root path
  if (event.httpMethod === 'GET' && (event.path === '/.netlify/functions/api' || event.path === '/.netlify/functions/api/')) {
    const response: ApiResponse = {
      status: 'SUCCESS',
      message: 'API is running',
      endpoints: {
        questions: '/.netlify/functions/api/data'
      }
    };
