...
exports.handler = async (event, context) => {
  ...
  if (event.httpMethod === 'GET' && event.path === '/.netlify/functions/api-v2/data') {
    try {
      console.log('Serving questions data from api-v2');
      return {
        statusCode: 200,
        headers,
        body: JSON.stringify({
          ...
          message: 'Questions fetched successfully from api-v2'
        })
      };
    } catch (error) {
  ...
  // Handle root path
  if (event.httpMethod === 'GET' && (event.path === '/.netlify/functions/api-v2' || event.path === '/.netlify/functions/api-v2/')) {
    return {
      statusCode: 200,
      headers,
      body: JSON.stringify({
        ...
        endpoints: {
          questions: '/.netlify/functions/api-v2/data'
        }
      })
    };
  }
